---
title: I thought TypeScript's type system was powerful. And then I tried Rust.
---
...
Rust's type system to TypeScript is like TypeScript's type system to JavaScript. I wouldn't have believed these worlds before Rust. Having a type system which actually co-exists with your code is incredibly delightful.

When a value claims to have some type in TypeScript, you can't be sure that it actually is that type. Any of the hundreds of function calls used to derive this value may have used an `as` assertion, and if this assertion is wrong then the type is invalid. If an invalid type is somewhere deep enough it can spread like the plague, but you won't know.
...
I am super grateful to Rust and Haskell for opening my eyes on type systems. I also recognize for all the work done on the TypeScript compiler. While it's far from perfect, I understand it's the way it is and has to be that way. There's not much TS can do since it compiles to the same old JavaScript. TypeScript it is an essential layer of protection on top of your JavaScript.

<note>

I realize this isn't true anymore. There are definitely type-safe languagues that
transpile to JavaScript, like [PureScript](https://www.purescript.org/) and [Gleam](https://github.com/gleam-lang/gleam).

But it seems that TypeScript's success was largely because it was familiar to JavaScript developers. It did not introduce new concepts like functional programming and immutability.
